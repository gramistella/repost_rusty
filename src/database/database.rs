...
use diesel::{delete, PgConnection, RunQueryDsl, select, table};
...
use crate::database::errors::MyDatabaseError;
...
impl Database {
    ...
    pub fn new(username: String, credentials: HashMap<String, String>) -> Result<Self, Error> {
        ...
        let database_url = if IS_OFFLINE { format!("postgres://{db_username}:{db_password}@192.168.1.101/dev") } else {format!("postgres://{db_username}:{db_password}@192.168.1.101/prod") };
        ...
        diesel::sql_query("CREATE TABLE IF NOT EXISTS user_settings (
        ...
        let user_exists = user_settings::table
            .filter(user_settings::username.eq(&username))
        ...
        diesel::sql_query("CREATE TABLE IF NOT EXISTS content_info (
        ...
        diesel::sql_query("CREATE TABLE IF NOT EXISTS queued_content (
            username TEXT NOT NULL,
            url TEXT NOT NULL,
            caption TEXT NOT NULL,
            hashtags TEXT NOT NULL,
        ...
        diesel::sql_query("CREATE TABLE IF NOT EXISTS published_content (
            username TEXT NOT NULL,
            url TEXT NOT NULL,
            caption TEXT NOT NULL,
            hashtags TEXT NOT NULL,
        ...
        diesel::sql_query("CREATE TABLE IF NOT EXISTS rejected_content (
            username TEXT NOT NULL,
            url TEXT NOT NULL,
            caption TEXT NOT NULL,
            hashtags TEXT NOT NULL,
        ...
        diesel::sql_query("CREATE TABLE IF NOT EXISTS failed_content (
            username TEXT NOT NULL,
            url TEXT NOT NULL,
            caption TEXT NOT NULL,
            hashtags TEXT NOT NULL,
        ...
        diesel::sql_query("CREATE TABLE IF NOT EXISTS video_hashes (
            username TEXT NOT NULL,
            original_shortcode TEXT NOT NULL,
        ...
        diesel::sql_query("CREATE TABLE IF NOT EXISTS duplicate_content (
            username TEXT NOT NULL,
            original_shortcode TEXT NOT NULL,
            PRIMARY KEY (original_shortcode)
        )").execute(conn).unwrap();
        ...
        diesel::sql_query("CREATE TABLE IF NOT EXISTS bot_status (
            username TEXT PRIMARY KEY,
            message_id BIGINT NOT NULL,
        ...
        let bot_status_exists = bot_status::table
            .filter(bot_status::username.eq(&username))
            .first::<InnerBotStatus>(conn)
            .is_ok();
...
impl DatabaseTransaction {
    pub fn load_user_settings(&mut self) -> Result<UserSettings, Error> {

        let user_settings = user_settings::table
            .filter(user_settings::username.eq(&self.username))
            .first::<UserSettings>(&mut self.conn)
            .unwrap();

        Ok(user_settings)
    }

    pub fn _save_user_settings(&mut self, user_settings: &UserSettings) -> Result<(), Error> {
    ...
    pub fn load_bot_status(&mut self) -> Result<BotStatus, Error> {
        let bot_status = bot_status::table
            .filter(bot_status::username.eq(&self.username))
            .first::<InnerBotStatus>(&mut self.conn)
            .unwrap();
    ...
    pub fn save_bot_status(&mut self, bot_status: &BotStatus) -> Result<(), Error> {
        ...
        diesel::update(bot_status::table)
            .filter(bot_status::username.eq(&self.username))
            .set(inner_bot_status)
            .execute(&mut self.conn)
            .unwrap();
    ...
    pub fn save_duplicate_content(&mut self, duplicate_content: DuplicateContent) -> Result<(), Error> {
        diesel::insert_into(duplicate_content::table)
    ...
    pub fn load_duplicate_content(&mut self) -> Result<Vec<DuplicateContent>, Error> {
        let duplicate_content = duplicate_content::table
            .filter(duplicate_content::username.eq(&self.username))
            .load::<DuplicateContent>(&mut self.conn)
            .unwrap();

        Ok(duplicate_content)
    }

    pub fn get_content_info_by_shortcode(&mut self, shortcode: &String) -> Result<ContentInfo, Error> {

        let found_content = content_info::table
            .filter(content_info::username.eq(&self.username))
            .filter(content_info::original_shortcode.eq(&shortcode))
            .first::<InnerContentInfo>(&mut self.conn).unwrap();
    ...
    pub fn remove_content_info_with_shortcode(&mut self, shortcode: &String) -> Result<(), Error> {
        
        delete(content_info::table)
            .filter(content_info::username.eq(&self.username))
            .filter(content_info::original_shortcode.eq(&shortcode))
            .execute(&mut self.conn)
            .unwrap();
    ...
    pub fn save_content_info(&mut self, content_info: &ContentInfo) -> Result<(), Error> {
    ...
    pub fn load_content_mapping(&mut self) -> Result<Vec<ContentInfo>, Error> {
        let content_list = content_info::table
            .filter(content_info::username.eq(&self.username))
            .load::<InnerContentInfo>(&mut self.conn)
            .unwrap();

        let content_list = content_list.iter().map(|content| {
            ContentInfo {
    ...
    pub fn get_temp_message_id(&mut self, user_settings: &UserSettings) -> u64 {
        ...
        let message_id_vec = content_info::table
            .select(content_info::message_id)
            .filter(content_info::username.eq(&self.username))
    ...
    pub fn remove_post_from_queue_with_shortcode(&mut self, shortcode: String) -> Result<(), Error> {
        use crate::database::database::queued_content::dsl::*;
        use diesel::prelude::*;

        let deleted_rows = diesel::delete(queued_content)
            .filter(original_shortcode.eq(&shortcode))
        ...
        if deleted_rows > 0 {
            ...
            let mut queued_posts: Vec<QueuedContent> = queued_content
                .filter(username.eq(&self.username))
                .order_by(will_post_at.asc())
                .load(&mut self.conn)?;

            let removed_post_index = queued_posts
                .iter()
    ...
    pub fn save_queued_content(&mut self, queued_post: &QueuedContent) -> Result<(), Error> {
    ...
    pub fn load_content_queue(&mut self) -> Result<Vec<QueuedContent>, Error> {
        let content_queue = queued_content::table
            .filter(queued_content::username.eq(&self.username))
            .order(queued_content::will_post_at)
            .load::<QueuedContent>(&mut self.conn)
            .unwrap();

        Ok(content_queue)
    }

    pub fn get_queued_content_by_shortcode(&mut self, shortcode: String) -> Result<QueuedContent, MyDatabaseError> {
        let content_queue = self.load_content_queue().unwrap();

        content_queue.iter().find(|&content| content.original_shortcode == shortcode).cloned().ok_or(MyDatabaseError::ContentNotFound)
    }

    pub fn get_rejected_content_by_shortcode(&mut self, shortcode: String) -> Result<RejectedContent, MyDatabaseError > {
        let rejected_content = self.load_rejected_content().unwrap();

        rejected_content.iter().find(|&content| content.original_shortcode == shortcode).cloned().ok_or(MyDatabaseError::ContentNotFound)
    }

    pub fn get_failed_content_by_shortcode(&mut self, shortcode: String) -> Result<FailedContent, MyDatabaseError> {
        let failed_content = self.load_failed_content().unwrap();

        failed_content.iter().find(|&content| content.original_shortcode == shortcode).cloned().ok_or(MyDatabaseError::ContentNotFound)
    }

    pub fn get_published_content_by_shortcode(&mut self, shortcode: String) -> Result<PublishedContent, MyDatabaseError> {
        let published_content = self.load_posted_content().unwrap();

        published_content.iter().find(|&content| content.original_shortcode == shortcode).cloned().ok_or(MyDatabaseError::ContentNotFound)
    }

    pub fn remove_rejected_content_with_shortcode(&mut self, shortcode: String) -> Result<(), Error> {
        delete(rejected_content::table)
            .filter(rejected_content::original_shortcode.eq(&shortcode))
            .filter(rejected_content::username.eq(&self.username))
            .execute(&mut self.conn)
            .unwrap();
    ...
    pub fn save_rejected_content(&mut self, rejected_content: RejectedContent) -> Result<(), Error> {
    ...
    pub fn load_rejected_content(&mut self) -> Result<Vec<RejectedContent>, Error> {
        let rejected_content = rejected_content::table
            .filter(rejected_content::username.eq(&self.username))
            .load::<RejectedContent>(&mut self.conn)
            .unwrap();
    ...
    pub fn save_published_content(&mut self, published_content: PublishedContent) -> Result<(), Error> {
        let queued_content = self.get_queued_content_by_shortcode(published_content.original_shortcode.clone());
        let mut removed = false;

        if let Ok(queued_content) = queued_content {
        ...
        if !removed {
            ...
            diesel::delete(queued_content::table)
        ...
        diesel::delete(published_content::table)
        ...
        diesel::insert_into(published_content::table)
    ...
    pub fn load_posted_content(&mut self) -> Result<Vec<PublishedContent>, Error> {
        let posted_content = published_content::table
            .filter(published_content::username.eq(&self.username))
            .load::<PublishedContent>(&mut self.conn)
            .unwrap();
    ...
    ///
    /// Will automatically remove the content from the content_queue
    pub fn save_failed_content(&mut self, failed_content: FailedContent) -> Result<(), Error> {
        // First we check if the content is actually in the content_queue
        let exists = queued_content::table
            .filter(queued_content::username.eq(&self.username))
            .filter(queued_content::original_shortcode.eq(&failed_content.original_shortcode))
            .execute(&mut self.conn)?;
        ...
        diesel::insert_into(failed_content::table)
            .values(&failed_content)
            .execute(&mut self.conn)?;

        Ok(())
    }
    
    pub fn load_failed_content(&mut self) -> Result<Vec<FailedContent>, Error> {
        
        let failed_content = failed_content::table
            .filter(failed_content::username.eq(&self.username))
            .load::<FailedContent>(&mut self.conn)
            .unwrap();
        
        Ok(failed_content)
    }

    pub fn get_new_post_time(&mut self) -> Result<String, Error> {
        let user_settings = self.load_user_settings()?;
    ...
    pub fn load_hashed_videos(&mut self) -> Result<Vec<HashedVideo>, Error> {
        
        let hashed_videos = video_hashes::table
            .filter(video_hashes::username.eq(&self.username))
            .load::<InnerHashedVideo>(&mut self.conn)
            .unwrap();
        
        let outer_hashed_video = hashed_videos.iter().map(|hashed_video| {
            HashedVideo {
        ...
        Ok(outer_hashed_video)
    }


    pub fn save_hashed_video(&mut self, hashed_video: HashedVideo) -> Result<(), Error> {
        ...
        diesel::insert_into(video_hashes::table)
            .values(&inner_hashed_video)
            .on_conflict(video_hashes::original_shortcode)
            .do_update()
            .set(&inner_hashed_video)
            .execute(&mut self.conn)
            .unwrap();
    ...
    fn shortcode_exists_in_table(&mut self, table_name: &str, shortcode: &str) -> bool {
        match table_name {
            "content_info" => {
                let exists_in_table = select(exists(content_info::table
                    .filter(content_info::original_shortcode.eq(shortcode))
                    .filter(content_info::username.eq(&self.username))))
            ...
            "published_content" => {
                let exists_in_table = select(exists(published_content::table
                    .filter(published_content::original_shortcode.eq(shortcode))
                    .filter(published_content::username.eq(&self.username))))
                    .get_result(&mut self.conn)
                    .unwrap();
                exists_in_table
            ...
            "queued_content" => {
                let exists_in_table = select(exists(queued_content::table
                    .filter(queued_content::original_shortcode.eq(shortcode))
                    .filter(queued_content::username.eq(&self.username))))
                    .get_result(&mut self.conn)
                    .unwrap();
                exists_in_table
            ...
            "rejected_content" => {
                let exists_in_table = select(exists(rejected_content::table
                    .filter(rejected_content::original_shortcode.eq(shortcode))
                    .filter(rejected_content::username.eq(&self.username))))
                    .get_result(&mut self.conn)
                    .unwrap();
            ...
            "failed_content" => {
                let exists_in_table = select(exists(failed_content::table
                    .filter(failed_content::original_shortcode.eq(shortcode))
                    .filter(failed_content::username.eq(&self.username))))
                    .get_result(&mut self.conn)
                    .unwrap();
                exists_in_table
            ...
            "duplicate_content" => {
                let exists_in_table = select(exists(duplicate_content::table
                    .filter(duplicate_content::original_shortcode.eq(shortcode))
                    .filter(duplicate_content::username.eq(&self.username))))
                    .get_result(&mut self.conn)
                    .unwrap();
                exists_in_table
    ...
    pub async fn clear_all_other_bot_statuses(&mut self) -> Result<(), Error> {
        delete(bot_status::table
            .filter(bot_status::username.ne(&self.username))).execute(&mut self.conn).unwrap();
