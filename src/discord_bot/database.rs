...
impl DatabaseTransaction {
    ...
    pub fn load_content_mapping(&mut self) -> Result<IndexMap<MessageId, ContentInfo>> {
        let tx = self.conn.transaction()?;
        let mut stmt = tx.prepare("SELECT username, message_id, url, status, caption, hashtags, original_author, original_shortcode, last_updated_at, url_last_updated_at, added_at, page_num, encountered_errors FROM content_info WHERE username = ?1 ORDER BY added_at")?;
        let content_info_iter = stmt.query_map([self.username.clone()], |row| {
            let username: String = row.get(0)?;
            let message_id: u64 = row.get(1)?;
            ...
            Ok((MessageId::new(message_id), content_info))
        })?;
    ...
    pub fn get_temp_message_id(&mut self, user_settings: UserSettings) -> i64 {
        ...
        let message_id_iter = stmt
            .query_map([self.username.clone()], |row| {
                let message_id: i64 = row.get(0).unwrap();
                Ok(message_id)
            })
            .unwrap();
        ...
        if let Some(removed_post_index) = queued_posts.iter().position(|post| post.original_shortcode == shortcode) {
            ...
            if removed_post_index <= queued_posts.len() {
                for post in queued_posts.iter_mut().skip(removed_post_index) {
                    ...
                    post.last_updated_at = (now_in_my_timezone(&user_settings) - INTERFACE_UPDATE_INTERVAL).to_rfc3339();
    ...
    pub fn save_queued_content(&mut self, queued_post: QueuedContent) -> Result<()> {
        let tx = self.conn.transaction()?;
